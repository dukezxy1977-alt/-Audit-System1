...
DEFAULT_RULEBOOK = WORKSPACE_DIR / "outputs" / "rule_engine_v10" / "工程项目招标投标审计规则引擎基础库_V10.xlsx"
DATA_DIR = BASE_DIR / "data"
UPLOAD_DIR = BASE_DIR / "uploads"
DB_PATH = DATA_DIR / "audit_engine.db"
...
app = Flask(__name__)
app.secret_key = "dev-audit-rule-engine"
...
def rows_by_code(rulebook_id, sheet_name, code):
    with db() as conn:
        rows = conn.execute(
            "SELECT row_json FROM rule_rows WHERE rulebook_id=? AND sheet_name=? AND rule_code=? ORDER BY row_index",
            (rulebook_id, sheet_name, code),
        ).fetchall()
...
def keywords_from_text(text):
    ...
    for token in tokens:
        terms.append(token)
        chinese_parts = re.findall(r"[\u4e00-\u9fff]{2,}", token)
...
def build_match_terms(rule, logic, evidence, project_text):
    candidates = []
    for field in ["违规情景", "负面行为"]:
        candidates.extend(keywords_from_text(rule.get(field, "")))
    for i in range(1, 5):
        candidates.extend(keywords_from_text(logic.get(f"判断条件{i}", "")))
...
def generate_risks(project_id):
    ...
    with db() as conn:
        for item in rules:
            ...
            evidence = row_by_code(rb["id"], "Evidence_Master", code)
            ...
            terms = build_match_terms(rule, logic, evidence, project_text)
            if len(terms) < 2:
                continue
    ...
    if request.method == "POST":
        ...
        if action == "default":
            if not DEFAULT_RULEBOOK.exists():
                flash("未找到默认 V10 Excel 文件，请先生成或上传规则库。", "error")
            else:
        ...
        if Path(file.filename).suffix.lower() not in ALLOWED_RULE_EXT:
            ...
            return redirect(url_for("import_rules"))
        stored = UPLOAD_DIR / f"rulebook_{uuid.uuid4().hex}.xlsx"
        file.save(stored)
...
if __name__ == "__main__":
    init_db()
    if not active_rulebook() and DEFAULT_RULEBOOK.exists():
        import_rulebook(DEFAULT_RULEBOOK, "工程项目招标投标审计规则引擎基础库_V10")
